...
Ext.ux.UniversalUI = Ext.extend(Ext.Panel, {
    ...
    /*-----------------------------------------------------------*
     *              Get the meaning of word onItemTap            *
     *-----------------------------------------------------------*/
    ...
    onNavPanelItemTap: function(subList, subIdx, el, e) {
        ...
        if (record) {
            ...
            var a;
            for(a = 0; a <= 10; a++)
                demos.ListStore.removeAt(a);
            //card        = record.get('card');
            // fixed card, avoid the type error caused by returned JSON
            card        = demos.List; 

            //anim        = record.get('cardSwitchAnimation');
            // fixed anim, avoid the type error caused by returned JSON
            anim        = 'cube';
            preventHide = record.get('preventHide');
            
            var word = record.get('text');
            // var url = "http://localhost:8080/getMeaning?word="+word;
            var url = "http://pydictionary.appspot.com/getMeaning?word="+word;
            ...
            $.get(
                    ...
                    function(data) {
                        ...
                        for (i = 0; i <= wordList.length; i++) {
                            ...
                            var e = new Object();
                            e.word = wordList[i].word;
                            demos.ListStore.add(e);
                        }
                    }   
            ); 
...
sink.Main = {
    ...
    onNavigate : function(ui, record) {
        if (record.data && record.data.source) {
            ...
            // fixed source, for reducing the size of structure.json
            var source = "src/demos/list.js";
